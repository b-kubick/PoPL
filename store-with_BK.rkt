...
(define-type-alias Store (Listof Storage)) ;defined as a list of storage so we can use first & rest
(define mt-store empty)
#;(define override-store cons) ;change the definition of override-store so that the old value is dropped

#|
Should take a storage object with (loc and val) & list of Storage obj as parameters
Should return a new Store or update the exsisting one with teh same loc
check if the list of storage is empty -> add the new Storage obj to empt store & return it (case when adding the first time)
if store not empty -> checkif the locof the first Storage obj matches the loc of obj being added/updated
   -> if loc match means it needs to be updated -> cons the new one to the rest and discard the old one
   -> if loc don't match -> keep first storage and recursive call override-store on the rest.
should continue until a matching loc is found or until the end is reached 
...
(define (interp [a : Exp] [env : Env] [sto : Store]) : Result
  (type-case Exp a
    ...
    [(beginE loExp)
     (beginE-helper loExp env sto)]))
#|
takes the inhalt of begin, current env and current sto as parameters
- when only 1 exp left  evaluate that (Base case)
- if empty -> error
- when multiple exp evaluate the first one in the current env and sto
use with like in the video

|#
(define (beginE-helper loExp env sto)
  (cond
    [(equal? (length loExp) 1) (interp (first loExp) env sto)]
    [(empty? loExp) (error 'box "List empty")]
    [else (with [(v-l sto-l)(interp (first loExp) env sto)]
                (beginE-helper (rest loExp) env sto-l))]))
...
(module+ test
  ...
  (test/exn (fetch 3 (override-store (cell 4 (numV 5)) ;fetching from unallocated loc
                                     mt-store))
            "unallocated location")
  (test/exn (interp (parse `[begin]) ;handle empty begin exp
                    mt-env
                    mt-store)
            "List empty"))
